...
internal class PendingRestore(
    ...
    private fun targetFile(path: String): File {
        return when {
            ...
            // Full ("everything") backups additionally carry the app data dir's siblings of
            // files/ — datastore/*.preferences_pb and shared_prefs/*.xml — where everything
            // configured outside settings.json lives (tool approvals, termux/browser/telegram/
            // notification prefs). Installing them through the same journal is what makes a
            // full restore atomic: it happens before Koin, Room, SettingsStore or any DataStore
            // consumer is initialized, so nothing can flush over the restored files.
            path.startsWith("appdata/") -> resolveInside(
                requireNotNull(filesDir.parentFile) { "Invalid restore path: $path" },
                path.removePrefix("appdata/"),
            )
